...
# Generate hierarchical clustering dendrogram from 1-correlation distance matrix
# using average linkage and save as PDF
function func_PlotDendrogram(samples::Vector{SampleStore}, corr::Matrix{Float64}, cfg::Config)
    n = length(samples)
    n == 0 && return
    n < 2 && return
    # Convert correlation to distance matrix
    dist_mat = 1.0 .- corr
    hc = hclust(dist_mat, linkage=:average)
    plt = plot(hc, xticks=(1:n, [s.name for s in samples]), xlabel="Samples", ylabel="Distance (1 - Pearson correlation)", legend=false, size=(800, 600), xrotation=45)
    savefig(plt, joinpath(cfg.outdir, cfg.out_prefix * ".pdf"))
end
...
function func_Main()
    ...
    func_PlotDendrogram(sample_accum, corr, cfg)
end
